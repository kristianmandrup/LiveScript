...
(function(){
  var LiveScript, LiveScriptEs6, path, fs, util, prelude, each, breakList, ref$, parseOptions, generateHelp, version, es6Version, say, warn, die, p, pp, ppp, o, positional, e, slice$ = [].slice;
  LiveScript = require('../..');
  LiveScriptEs6 = require('./index');
  path = require('path');
  fs = require('fs');
  util = require('util');
  ...
  es6Version = LiveScriptEs6.VERSION;
  say = console.log;
  warn = console.error;
  ...
  function fileContains(file, expr){
    return expr.test(slurp(file));
  }
  function slurp(file){
    return fs.readFileSync(file).toString();
  }
  function detectLivescriptEs(filename){
    var isHarmonyFile;
    isHarmonyFile = fileContains(filename, /^("use harmony"|'use harmony');?\s*\n/);
    say("is-harmony-file", isHarmonyFile);
    if (isHarmonyFile) {
      return LiveScriptEs6;
    } else {
      return LiveScript;
    }
  }
  function compileScript(filename, input, base){
    ...
    LiveScript = detectLivescriptEs(filename);
    options = {
      filename: filename,
      bare: o.bare,
      'const': o['const']
