...
(function(){
  var LiveScript, path, fs, util, prelude, each, breakList, ref$, parseOptions, generateHelp, version, say, warn, die, p, pp, ppp, o, positional, e, slice$ = [].slice;
  LiveScript = require('..');
  path = require('path');
  fs = require('fs');
  util = require('util');
  ...
  function writeJS(source, js, base, json){
    var isExe, ext, extension, filename, dir, that, jsPath;
    isExe = fileContains(source, /^#!\/.+node\s*\n/);
    if (o.exe || isExe) {
      ext = 'X';
    }
    if (json) {
    ...
    extension = ext === void 8 ? '.js' : ext;
    filename = path.basename(source).replace(/(?:(\.\w+)?\.\w+)?$/, function(){
      return arguments[1] || extension;
    });
    ...
    function compile(){
      fs.writeFile(jsPath, js || '\n', function(e){
        ...
        if (o.exe || isExe) {
          fs.chmod(jsPath, '0755', function(xe){
            if (xe) {
              return warn(xe);
            }
          });
